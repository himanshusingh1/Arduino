...
//    _setWindow(x,y,w,h);
    int count = 0;
    for (int down = y;down < y+h;down++){
        for (int right = x;right<(x+w);right++){
//            if (bitmap[count] != 0x00){
//                drawPixel(right,down,color);
//            }else{
//                drawPixel(right,down,COLOR_BLACK);
//            }
            _setWindow(right, down, right+1, down+1);
            _writeData(color >> 8, color);
            count++;
        }
    }
